    ...
    <div id="awards-container">
      ...
      <div class="award-item">
        <div class="form-group">
          ...
          <input type="text" id="award_name" name="awards[0][name]" class="form-control" required value="{{$award->name}}">
        </div>
        ...
        <div class="form-group" id="awards[0][condition_type_div]">
          ...
          <select name="awards[0][condition_type]" class="form-control awards[0][condition_type]" required>
            <option {{$award->type == 'EXACT_POINT' ? '' : 'selected'}} value="EXACT_POINT">Ponto Exato</option>
            <option {{$award->type == 'LOWEST_POINT' ? '' : 'selected'}} value="LOWEST_POINT">Pé Frio</option>
            <option {{$award->type == 'WINNER' ? '' : 'selected'}} value="WINNER">Vencedor</option>
          </select>
        </div>

        <div class="form-group" id="awards[0][exact_point_div]">
          <label for="awards[0][exact_point_value]">Quantos Pontos? (se aplicável):</label>
          <input type="number" name="awards[0][exact_point_value]" class="form-control" value="{{$award->exact_point_value}}">
        </div>
        ...
        <div class="form-group" id="awards[0][winner_point_div]">
          ...
          <input type="number" name="awards[0][winner_point_value]" class="form-control" value="{{$award->winner_point_value}}">
        </div>

        <div class="form-group awards[0][value_point_div]">
          <label for="awards[0][amount]">Valor do Prêmio:</label>
          <input type="number" name="awards[0][amount]" class="form-control" step="0.01" value="{{$award->amount}}" required>
        </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateDivVisibility(awardIndex) {
      const conditionTypeSelect = document.querySelector(`[name="awards[${awardIndex}][condition_type]"]`);
    ...
    document.getElementById('add-award').addEventListener('click', function() {
      ...
      newAward.querySelector(`select[name="awards[${awardIndex}][condition_type]"]`).addEventListener('change', function() {
