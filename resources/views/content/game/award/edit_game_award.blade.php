      ...
      <form action="{{ route('edit-game-award', $gameAward->id) }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <textarea
            ...
            disabled
            rows="3"
            required>{{ old('game_name', $gameAward->game->name) }}</textarea>
        </div>
        ...
        <div class="mb-3">
          ...
          <textarea
            ...
            name="name"
            rows="3"
            required>{{ old('name', $gameAward->name) }}</textarea>
        </div>
        ...
        <div class="mb-3">
          ...
          <select class="form-select" id="condition_type" name="condition_type" required>
            <option {{ old('condition_type', $gameAward->condition_type) == 'EXACT_POINT' ? 'selected' : '' }} value="EXACT_POINT">Ponto Exato</option>
            <option {{ old('condition_type', $gameAward->condition_type) == 'LOWEST_POINT' ? 'selected' : '' }} value="LOWEST_POINT">Pé Frio</option>
            <option {{ old('condition_type', $gameAward->condition_type) == 'WINNER' ? 'selected' : '' }} value="WINNER">Vencedor</option>
          </select>
        </div>

        <div class="mb-3" id="exact_point_div">
          <label for="exact_point_value" class="form-label">Quantos pontos para o premio? (se aplicável):</label>
          <input
            type="number"
            class="form-control"
            id="exact_point_value"
            name="exact_point_value"
            value="{{ old('exact_point_value', $gameAward->exact_point_value) }}">
        </div>

        <div class="mb-3" id="winner_point_div">
          <label for="winner_point_value" class="form-label">Quantos pontos para o vencedor? (se aplicável):</label>
          <input
            type="number"
            class="form-control"
            id="winner_point_value"
            name="winner_point_value"
        ...
        <div class="mb-3">
          ...
          <input
            ...
            id="amount"
            name="amount"
            value="{{ old('amount', $gameAward->amount) }}">
        </div>
        ...
        <a href="{{ route('show-game', ['id' => $gameAward->game_id]) }}" class="btn btn-secondary">Cancelar</a>
      </form>
    </div>
  </div>
</div>
